...
AFTER UPDATE ON rooms
FOR EACH ROW 
WHEN ((NEW.p1status IS DISTINCT FROM OLD.p1status AND NEW.p2status IS DISTINCT FROM OLD.p2status) AND (NEW.p1status IS DISTINCT FROM null AND NEW.p2status IS DISTINCT FROM null))
